...
function eventTotals(ev) {
  ...
  if (isCommission) {
    ...
    income = workers.reduce((s, w) => s + (Number(w.count) || 0) * (Number(w.clientPayEach) || 0), 0);
    manpower = workers.reduce((s, w) => s + (Number(w.count) || 0) * (Number(w.payoutEach) || 0), 0);
  } else {
  ...
  return { expTotal, travelTotal, manpower, income, totalCost, received, netActual: received - totalCost };
}
...
function eventsSummary() {
  return allEventTotals().reduce((acc, { t }) => ({
    received: acc.received + t.received, cost: acc.cost + t.totalCost, netActual: acc.netActual + t.netActual,
  }), { received: 0, cost: 0, netActual: 0 });
}
...
function thisMonthReceived() {
  const m = thisMonthLabel();
  return allEventTotals().filter(({ ev }) => monthKey(ev.date) === m).reduce((s, { t }) => s + t.received, 0);
}
function monthlyBreakdown() {
  const map = {};
  allEventTotals().forEach(({ ev, t }) => {
    const gk = monthGroupKey(ev.date);
    if (!gk) return;
    ...
    <div class="container">
      <div class="header-row">
        ...
        <div class="header-actions">
          ...
          <button class="icon-btn" data-action="toggle-overview" title="Combined net worth">${state.showOverview ? ICONS.eyeOff : ICONS.eye}</button>
        </div>
      </div>

      <div class="ticker">
        <span>This month</span>
        <span class="tk-in"><b>In ${fmt(thisMonthReceived(), state.currency)}</b></span>
        <span class="tk-out"><b>Out ${fmt(thisMonthPersonal(), state.currency)}</b></span>
        <span>Events net <b style="color:${summary.netActual >= 0 ? "var(--green)" : "var(--red)"}">${fmt(summary.netActual, state.currency)}</b></span>
      </div>

      ${state.showMonthly ? renderMonthlyPanel() : ""}
      ${state.showOverview ? renderOverview(summary) : ""}
...
function renderOverview(summary) {
  const combined = summary.netActual - personalTotal();
  return `
  <div class="overview-panel">
    <div class="overview-row"><span>Events net (received − cost)</span><span style="color:${summary.netActual >= 0 ? "var(--green)" : "var(--red)"}">${fmt(summary.netActual, state.currency)}</span></div>
    ...
    <div class="card-body">
      <div class="card-top">
        <div class="card-fields">
          ...
          <div class="meta-row">
            <input type="date" data-field="date" data-event-id="${ev.id}" value="${esc(ev.date)}" />
      ...
      <div class="sub-panel" style="border-top:none;border:1px solid var(--border-soft);border-radius:9px;margin-top:8px;">
        ...
        ${(ev.workers || []).map((w) => {
          ...
          const marginEach = clientEach - payEach;
          const marginTotal = marginEach * count;
          return `
...
function addEvent() {
  state.events.unshift({
    id: uid(), name: "New Event", date: new Date().toISOString().slice(0, 10), client: "",
...
document.addEventListener("click", (e) => {
  ...
  if (action === "add-worker") { const ev = findEvent(btn.dataset.eventId); if (ev) { ev.workers = ev.workers || []; ev.workers.push({ id: uid(), name: "", count: 1, clientPayEach: 0, payoutEach: 0 }); persist(); render(); } return; }
...
function exportToExcel() {
  ...
  const evRows = [...state.events].sort((a, b) => new Date(a.date) - new Date(b.date));
  const evHeader = ["Date", "Event", "Type", "Client", "Income / Client Total", "Manpower Cost", "Other Expenses", "Travel Cost", "Total Cost", "Net / Commission", "Payment Status"];
  const evData = evRows.map((ev) => {
    const t = eventTotals(ev);
    return [
      ev.date, ev.name, ev.type === "commission" ? "Manpower/Commission" : "My Event", ev.client || "",
      t.income, t.manpower, t.expTotal, t.travelTotal, t.totalCost, t.netActual, ev.paymentStatus,
    ];
  });
  const wsEvents = XLSX.utils.aoa_to_sheet([evHeader, ...evData]);
  wsEvents["!cols"] = [{ wch: 12 }, { wch: 22 }, { wch: 18 }, { wch: 16 }, { wch: 16 }, { wch: 14 }, { wch: 14 }, { wch: 12 }, { wch: 12 }, { wch: 14 }, { wch: 12 }];

  const workerHeader = ["Date", "Event", "Name", "Count", "Client Pays Each", "You Pay Each", "Margin Each", "Margin Total"];
  const workerData = [];
  evRows.filter((ev) => ev.type === "commission").forEach((ev) => {
    (ev.workers || []).forEach((w) => {
      ...
      workerData.push([ev.date, ev.name, w.name || "", count, clientEach, payEach, clientEach - payEach, (clientEach - payEach) * count]);
    });
  });
  const wsWorkers = XLSX.utils.aoa_to_sheet([workerHeader, ...workerData]);
  wsWorkers["!cols"] = [{ wch: 12 }, { wch: 22 }, { wch: 16 }, { wch: 8 }, { wch: 14 }, { wch: 14 }, { wch: 12 }, { wch: 12 }];
